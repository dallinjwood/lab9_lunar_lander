...
void lander_init() {
  ...
  the_lander.location.x = 160;
  the_lander.location.y = 0;
  the_lander.vertical_velocity = 0.98;
  the_lander.horizontal_velocity = 0;
}

// calculates the x and y thrust values given the current angle
double get_thrust_y() { return sin_degrees(the_lander.angle); }

double get_thrust_x() { return cos_degrees(the_lander.angle); }
